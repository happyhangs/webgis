    ...
    return (
      <div className="panel layer-panel collapsed">
        <div className="panel-header panel-header-vertical">
          <button className="panel-toggle" onClick={() => setCollapsed(false)} title="展开图层面板">
  ...
  return (
    <div className="panel layer-panel">
      <div className="panel-header">
        <Layers size={15} />
        <span>图层</span>
        <button className="panel-header-btn" onClick={handleAddLayer} title="新建图层"><Plus size={15} /></button>
        <button className="panel-toggle" onClick={() => setCollapsed(true)} title="最小化"><ChevronsLeft size={14} /></button>
      </div>
      <div className="panel-body">
          ...
          return (
            <div key={layer.id} className={`layer-group ${isActive ? 'active' : ''}`}>
              <div className="layer-header">
                <button className="layer-chevron" onClick={() => toggleExpand(layer.id)}>
                ...
                <button className="layer-action-btn" title="重命名" onClick={() => startRename(layer.id)}><Pencil size={12} /></button>
                {isDefault && features.length > 0 && (
                  <button className="layer-action-btn danger" title="清空默认图层"
                    onClick={() => { if (confirm(`确定清空默认图层全部 ${features.length} 个标注？`)) dispatch({ type: 'CLEAR_LAYER_FEATURES', layerId: DEFAULT_LAYER_ID }); }}>
                    <Trash2 size={12} /></button>
                )}
                {!isDefault && (
                  <button className="layer-action-btn danger" title="删除图层"
        ...
        <div className="builtin-section">
          <div className="builtin-header"><Database size={13} /><span>内置数据</span></div>
            ...
            return (
              <div key={cat.key} className="builtin-cat">
                ...
                {catLayers.map((def) => {
                  ...
                  return (
                    <div key={def.key} className={`builtin-item ${alreadyLoaded ? 'loaded' : ''}`}
                      onClick={() => { if (!alreadyLoaded && !loading) handleLoadBuiltin(def); }}
                      title={alreadyLoaded ? '已加载' : `点击加载 (${def.size})`}>
                      <span className="builtin-color" style={{ backgroundColor: def.color }} />
